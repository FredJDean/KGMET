...
DS_DICT = {
    ...
    "mquake": (MQuAKEDataset, compute_rewrite_quality_counterfact)
}
...
def main(
    ...
    if alg_name == "KGMET":
        ...
        gnn_model = GNN(
            ...
            use_cuda=True,
            analysis=False)
        file_path = "null_space_project_{}.pt".format(hparams.model_name)
        if os.path.exists(file_path):
            P = torch.load(file_path)
            print(f"Loaded existing file: {file_path}")
        else:
            for i, layer in enumerate(hparams.layers):
                P[i,:,:] = get_project(model,tok,layer,hparams)
            torch.save(P, file_path)
            print("Computed P")
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        graph_input =  [item['triples'] for item in graphds if item['case_id'] in case_ids]
        ...
        if any(alg in alg_name for alg in ["KGMET", "MEMIT_seq", "NSE"]):
            if ds_name == "mquake":
                edited_model, weights_copy = apply_algo(
                        ...
                        tok,
                        [
                            {"case_id": record["case_id"], **sub_request}
                            for record in record_chunks
                            for sub_request in record["requested_rewrite"]
                        ],
                        hparams,
                        **glame_args,
                        **args_conserve_memory,
